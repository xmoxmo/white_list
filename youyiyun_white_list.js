/*
new Env('优亦云IP白名单');
35 0-23 * * * youyiyun_white_list.js
by:xmo
优亦云白名单自动替换
WxPusher一对一：设置WP_APP_TOKEN_ONE和WP_APP_MAIN_UID自动启动
*/

// 定义upackid(用户套餐ID)和token(用户token)
let upackid = '';
let token = '';
if (process.env.YOUYIYUN_UPACKID) {
  upackid = process.env.YOUYIYUN_UPACKID;
}
if (process.env.YOUYIYUN_TOKEN) {
  token = process.env.YOUYIYUN_TOKEN;
}
//console.log(upackid + '\n' + token)
...
if (upackid == '') {
  console.log('请先定义export YOUYIYUN_UPACKID=(upackid)');
  process.exit(0);
}
if (token == '') {
  console.log('请先定义export YOUYIYUN_TOKEN=(token)');
  process.exit(0);
}
...
const ipFileName = 'youyiyunIp.txt';
...
async function addIpToWhiteList(currentIP) {
  const addIpUrl = `http://data.yyyip.cn:88/whiteip_api?method=add&token=${token}&upackid=${upackid}&ip=${currentIP}`;
  try {
    ...
    title = successCondition ? "优亦云白名单更换成功 ✅" : "优亦云白名单更换失败 ❌"; 
    ...
    return { success: false, title: "优亦云白名单更换失败 ❌", message };
  }
}

// 获取白名单IP
async function getwhiteip() {
  const getIpUrl = `http://data.yyyip.cn:88/whiteip_api?method=list&token=${token}`;
...
async function delwhiteip(oldip) {
  const delIpUrl = `http://data.yyyip.cn:88/whiteip_api?method=del&token=${token}&ip=${oldip}`;
...
async function main() {
  ...
  if (currentIP) {
    ...
    resultMessage = { success: false, title: "优亦云获取公网IP失败 ❌", message: "💡 获取公网IP返回空信息，终止执行！" };
    await sendNotification(resultMessage);
    const wxpusherResponse = await wxpusherNotify(
      resultMessage.title,
      resultMessage.message
