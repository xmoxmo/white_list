...
console.log("   *加载sendNotify，当前版本: 20230712[20240715]+fs202405081330");
// =======================================go-cqhttp通知设置区域===========================================
                ...
                try {
                    if (err) {
                        ...
                        if (data.StatusCode === 0) {
                            ...
                            console.log('飞书发送通知消息出现异常。');
                            console.log('  *异常信息：' + `${data.msg}\n`);
                        }
                    }
                } catch (e) {
                    $.logErr(e, resp);
...
function wxpusherNotifyByOne(text, desp, strsummary = "") {
    return new Promise((resolve) => {
        if (WP_APP_TOKEN_ONE) {
            var WPURL = "";
            if (process.env.WP_APP_ONE_URL) {
                WPURL = process.env.WP_APP_ONE_URL;
            }
